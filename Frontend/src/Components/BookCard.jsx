...
export default function BookCard({ book, onAddToCart }) {
  ...
  return (
    ...
    <Link
      ...
      className="font-jura flex flex-col items-center p-3 sm:p-4 sm:rounded-3xl hover:shadow-2xl hover:shadow-[#DBBCDB] transition duration-300 w-full mx-auto flex-shrink-0 relative group mt-4"
    >
      {/* Book Image */}
      <div className="w-full h-[180px] sm:h-[192px] overflow-hidden rounded-lg mb-2">
      ...
      <div className="w-full text-center mt-2" style={{ fontFamily: "Jura, sans-serif" }}>
        <h3 className="text-[#1F1E3E] text-base sm:text-md font-bold truncate px-1">{Title}</h3>
        <p className="text-[#B787B6] text-xs sm:text-sm mt-0.5 mb-2 truncate px-1">{Author}</p>
      </div>

      {/* Price */}
      <div className="flex items-baseline justify-center gap-2 mb-3">
        <span className="text-lg sm:text-lg font-bold text-[#1F1E3E]" style={{ fontFamily: "Jura, sans-serif" }}>
          ₹{Price}
        </span>
      </div>
